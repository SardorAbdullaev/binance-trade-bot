...
class Strategy(AutoTrader):
    ...
    def transaction_through_bridge(self, pair):
        ...
        if result is not None:
            ...
            btc_value = self.manager.get_ticker_price(coin)
            self.update_mongodb_last_buy_price(coin, btc_value, result.cumulative_quote_qty)
            self.logger.info(
                f"{coin}: Last buy price of {btc_value} in total amount of {result.cumulative_quote_qty} is persisted")
            return result
